...
class GrubTamerWindow(Adw.ApplicationWindow):
    ...
    def create_row(self, key, label, desc, opt_type, example=None, is_custom=False):
        ...
        # --- THEME SELECTOR LOGIC ---
            ...
            # Smart Dropdown for Themes
            themes = self.get_available_themes()
            model = Gtk.StringList.new(themes)
            dropdown = Gtk.DropDown(model=model)
            dropdown.set_valign(Gtk.Align.CENTER)

            # Determine selection based on current path
            # Current val might be "/boot/grub/themes/MyTheme/theme.txt"
            ...
            # Launch Editor for CURRENTLY SELECTED theme in dropdown
            ...
            # Store tuple so save logic knows how to handle it
            ...
            if example: widget.set_placeholder_text(f"e.g. {example}")
            row.add_suffix(widget)
            self.widget_map[key] = widget
